...
<div class = "header">
	<center> <h1> Modifica anvelope </h1> </center>
</div>
	...
	<?php // conexiune BD
		...
		if(isset($_POST['Modifica']) && ($_POST['nume'] != NULL))
		{
			...
			{ 
				echo "<center> <b> Numelele nu a fost gasit!</b></center><br><br>";
			}
			else
			{
				if($aderenta != NULL)
				{
				$query_update = "UPDATE Anvelope SET Durabilitate = '{$durabilitate}' WHERE Nume = '{$nume}'";
				$result = sqlsrv_query($conn, $query_update);
				}
				if($durabilitate != NULL)
				{
					$query_update = "UPDATE Anvelope SET Aderenta = '{$aderenta}' WHERE Nume = '{$nume}'";
					$result = sqlsrv_query($conn, $query_update);
				}
				if($aderenta == 'NULL')
				{
				$query_update = "UPDATE Anvelope SET Durabilitate = NULL WHERE Nume = '{$nume}'";
				$result = sqlsrv_query($conn, $query_update);
				}
				if($durabilitate == 'NULL')
				{
					$query_update = "UPDATE Anvelope SET Aderenta = NULL WHERE Nume = '{$nume}'";
					$result = sqlsrv_query($conn, $query_update);
				}
				
				echo "<center> <b> Anvelopele au fost modificate! </b> </center>";
			}
		}
		else if(isset($_POST['Modifica']) && ($_POST['nume'] == NULL))
		{
			
			echo "<center><b> Introduceti numele anvelopelor </b></center>";
		}
