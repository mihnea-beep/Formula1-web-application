...
<div class = "header">
	...
	<body>
			...
			if($view_mode == 1)
		{
			...
			$query_motor = "SELECT Nume, Capacitate
								  FROM Motoare
								  WHERE Nume = '{$car}'";
			
			$result = sqlsrv_query($conn, $query_motor);
			
			if(sqlsrv_has_rows($result) != 1){ # if there's no result, insert data into table
				echo "<center><p><b> Nume invalid </b></p></center>";
				$car = "invalid";
				$view_mode = 0;}
			else{
				$view_mode = 1;
				...
				echo "<th> <b> Nume </b> </th> <th> <b> Capacitate </b> </th>";
		...
		if($view_mode == 0)
		{
			...
			$result_motor = sqlsrv_query($conn, $query_motor);
			
			while($row = sqlsrv_fetch_array($result_motor)){
		...
		<form autocomplete = "off" method = "post" action = "<?php echo $_SERVER['PHP_SELF'];?>">
			...
			<div class="autocomplete" style="width:300px;">
				<input id="myInput" type="text" name="anvelope" placeholder="Nume motor">
			</div>
